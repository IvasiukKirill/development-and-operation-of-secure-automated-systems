...
static sgx_status_t SGX_CDECL sgx_foo(void* pms)
{
	...
	//
	sgx_lfence(); 
	//функцияпредставляет собой инструкцию ассемблера, которая гарантирует, что все инструкции, 
	//предшествующие ей, будут выполнены до того, как начнется выполнение инструкций, следующих за ней
	...
	if (_in_buf) {
		if (memcpy_s(_tmp_buf, _len_buf, _in_buf, _len_buf)) {  //Если _in_buf не является нулевым указателем, то происходит копирование данных из _in_buf в _tmp_buf с помощью функции memcpy_s
...
err:
	if (_in_buf) free(_in_buf); //освобождение памяти
	return status;
}
